...
export default {
  ...
  methods: {
    ...
    processFile(file) {
      ...
      // 检查文件大小（限制为10MB，避免localStorage溢出）
      const maxSize = 10 * 1024 * 1024 // 10MB
      if (file.size > maxSize) {
        alert('视频文件过大（超过10MB），请选择较小的视频文件')
        return
      }
      ...
      const gradients = [
        ...
        'linear-gradient(135deg, #ff69b4 0%, #9370db 50%, #48d1cc 100%)'
      ]
      ...
      reader.onload = (e) => {
        ...
        const videoData = {
          ...
          fileSize: file.size,
          fileType: file.type,
          videoData: videoDataUrl // 保存base64编码的视频数据
        }
