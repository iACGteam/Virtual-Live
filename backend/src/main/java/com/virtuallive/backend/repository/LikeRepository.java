...
public interface LikeRepository extends JpaRepository<Like, Integer> {
    ...
    // 统计用户点赞的内容数
    long countByUserAndContentType(User user, Like.ContentType contentType);
    ...
    @Query("SELECT v FROM Like l, Video v JOIN FETCH v.author WHERE l.contentId = v.postId AND l.contentType = :contentType AND l.user.userId = :userId AND v.isDeleted = false AND (v.tags IS NULL OR v.tags NOT LIKE '%__PRIVATE__%') ORDER BY l.createdAt DESC")
